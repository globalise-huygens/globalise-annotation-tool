...
const allowed_file = process.env.TOOL_ALLOWED_FILE;
const inc2lang2doc_file = data_dir + "/structured/inc2lang2doc_index.json";
const inc2str_file = data_dir + "/structured/inc2str_index.json";
const type2inc_file = data_dir + "/structured/type2inc_index.json";
const proj2inc_file = data_dir + "/structured/proj2inc_index.json";
const labels_file = data_dir + "/structured/labels.json";
...
const LL_DIR = data_dir + "/lexical_data/typicality/lexical_lookup/";
const NAF_DIR = data_dir + "/unstructured/";
...
fs.readFile(dynamic_lexicon_file, "utf8", function (err, data) {
    if (err) throw err; // we'll not consider error handling for now
...
passport.use(
    new LocalStrategy(function (username, password, done) {
        fs.readFile(allowed_file, "utf8", function (err, data) {
            if (err) throw err; // we'll not consider error handling for now
...
function readNAFFile(json_data, doc_name, inc_type) {
    var typfile =
        data_dir +
        "/lexical_data/typicality/typicality_scores/" +
        inc_type +
        ".json";
...
function handleFrameAnnotation(json_data, task_data, session_id) {
    if (task_data.fan_task == 1) {
        ...
        fs.readFile(suggestions_file, "utf8", function (err, data) {
...
function storeDynamicLexicon() {
    ...
    fs.writeFileSync(dynamic_lexicon_file, data, function (err, data) {
        if (err) {
            console.log(err);
        }
    });
