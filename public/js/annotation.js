...
$(function () {
    ...
    $("#fan-typical-select").slider({
        ...
        slide: function (e, u) {
            ...
            var selected = $('span[data-pred-status="system"]').filter(
                function () {
    ...
    $(document).on("click", "span.markable", function () {
        ...
        if (current_task == "1") {
            var corrected =
                $(t_selector).data("term-type") == "multiword" ||
                $(t_selector).data("term-type") == "compound";
        ...
        // Currently annotating frames
            ...
            if ($("#fan-task-select").val() == "1") {
                    ...
                    if ($(t_selector).hasClass("marked")) {
                        ...
                        if (lemma != undefined) {
                            loadFrames(type, language, lemma, function (data) {
                                renderDropdownWithGroups(
                                    "#fan-type-select",
                                    data,
                                    ["lu", "definition", "framenet"],
                                    "-Select-"
                                );
                            });
                        }
                ...
                if ($(t_selector).hasClass("coref")) {
                    ...
                    $('span[data-ref-id="' + ref_id + '"]').toggleClass(
                        "coref-marked"
                    );
...
function resetMCNPanel() {
    $("#ip-mcn-subdiv").empty();
    $("#ip-mcn-subdiv").append(
        "<tr><th>Token</th><th>Lemma</th><th>POS</th><th>Head</th></tr>"
    );
...
function resetPREPanel() {
    ...
    $("#ip-pre-rol").empty();
    $("#ip-pre-rol").append(
        "<tr><th>Frame Element</th><th>Role Type</th><th>Annotated</th><th>Expressed</th></tr>"
    );
...
function updateTask(clear) {
    if (enforced_role_annotation) {
        $("#annotation-task-selection").click(function (e) {
            ...
            if (
                confirm(
                    "Not all core roles are annotated, are you sure you want to continue?"
                )
            ) {
...
function updateMCNType() {
    ...
    if (
        $("#mcn-type-select").val() == "1" ||
        $("#mcn-type-select").val() == "2"
    ) {
        $("#ip-mcn").hide();
                ...
                $("#fea-role-select > optgroup > option").each(function () {
                    for (i in annotations["fea"]) {
                        for (j in annotations["fea"][i]) {
                            if (
                                $(this).val() ==
                                annotations["fea"][i][j]["premon"] &&
                                rpr_id == annotations["fea"][i][j]["predicate"]
                            ) {
    ...
    for (var i in subdivisions) {
        ...
        if (token != "") {
            ...
            var lem_input =
                "<input id=" +
                id +
                '_l type="text" class="w-100", value="' +
                token +
                '">';
            var pos_input =
                "<select id=" +
                id +
                '_p type="text" class="w-100">' +
                pos_options +
                "</select>";
                ...
                if (new_load || current_task == "2" || current_task == "3") {
                    ...
                    for (var key in annotations.fea) {
                        for (key2 in annotations.fea[key]) {
                            ...
                            predicate_info[r_info.predicate].roles.push(
                                r_info.premon
                            );
                        }
                    }
                    ...
                    loadMultipleRoles(predicate_prems, function (role_data) {
                        ...
                        for (pr_id in predicate_info) {
                            ...
                            if (p_info.status == "manual") {
                                for (var i in r_info) {
                                    if (
                                        p_info.roles.indexOf(r_info[i].value) >
                                        -1
                                    ) {
                                        overlap += 1;
                                    }
    ...
    for (var i in terms) {
        ...
        if (term.t_select in annotations["fea"]) {
            ...
            for (j in annotations["fea"][term.t_select]) {
                ...
                term.role_pred.push(
                    annotations["fea"][term.t_select][j].predicate
                );
            }
...
function renderDocument(doc_data, annotations) {
    ...
    var source =
        "https://www.nationaalarchief.nl/onderzoeken/archief/1.04.02/invnr/" +
        inv_number +
        "/file/" +
        file_name;
...
function renderStructuredData(incident_id, incident_txt, data) {
    ...
    result +=
        ...
        '" data-uri="' +
        incident_id +
        '" data-type="event" class="structured-data" target="_blank">' +
        incident_txt +
        "</a>";
    result += "<br/>";
    ...
    for (var property in data) {
        ...
        for (var i = 0; i < prop_vals.length; i++) {
            ...
            if (clean_property != "hasTimeStamp") {
                functionality =
                    'class="structured-data" data-uri="' +
                    cur_uri +
                    '" data-type="entity"';
            }
...
function renderDropdownWithGroups(
    element_id,
    items,
    data_items,
    default_option
    ...
    element.empty();
    element.append(
        $('<option value="None" selected>' + default_option + "</option>")
    );
...
function loadNAFFile(type, incident, document, callback) {
    ...
    $.get("/load_document", get_data, function (result, status) {
        ...
        if (e.status == 423) {
            ...
            printMessage(
                "Something went wrong while loading the requested document.",
                "error"
            );
        }
    });
...
function validateCorrection() {
    ...
    var correction_original = $(".marked")
        .clone()
        .children()
        .remove()
        .end()
    ...
    if (correction_task == "1") {
        ...
        // Compounds
            ...
            if (correction_subdivisions.join("") != correction_original) {
                return [
                    false,
                    "Compound components should recreate entire compound",
                ];
            }
    ...
    // Create
        ...
        for (var i in selected) {
            ...
            if (term.data("pred-id") && term.data("pred-status") != "system") {
                return [
                    false,
                    "One or more selected markables is already annotated",
                ];
            }
        }
...
function validateRoleAnnotation() {
    ...
    if (task == "1") {
        ...
        if (!(selected.length > 0)) {
            if (
                !confirm(
                    "Are you sure you want to annotate Frame Element as unexpressed?"
                )
            ) {
...
var validateReference = function () {
    ...
    if (task == "1") {
        ...
        var search_uri = $("#cor-uri-input").val()
        if (search_uri != "") {
            var search_lab = $("#cor-lab-input").val()

            var task_data = {
                cor_task: 1,
                target_ids: selected,
                referents_data: [{ uri: search_uri, type: "entity" }],
                sde_task_data: {
                    action: 1,
                    relation: "sem:hasActor",
                    wdt_uri: search_uri,
                    label: search_lab,
                }
            };
        ...
        // Get all selected markables
        var selected = getSelected();
        var referent = $('span[term-selector="' + selected[0] + '"]').data(
            "ref-uri"
        );
...
var validateStructuredData = function () {
    ...
    if (action == "None") {
        ...
        var wdt_uri = $("#cor-uri-input").val();
        var label = $("#cor-label-input").val();
...
function storeAndReload(task_data) {
    ...
    $.post("/store_annotation", request_data)
        ...
        .fail(function (err) {
            printMessage(
                "There was an error while saving your annotations",
                "warning"
            );
        });
}
...
function updateCORUri() {
    var query_time = new Date().getTime();
    var search_query = $("#cor-lab-input").val();
    $("#cor-uri-input").val("");

    if (search_query.length < 3) {
        $("#cor-lab-results").hide();
    } else {
        $.getJSON(
            `http://grlc.io/api-git/globalise-huygens/grlc-queries/search_ships?search=${search_query}&endpoint=https%3A%2F%2Fglobalise-data.diginfra.net%2Frepositories%2FGLOBALISE-DATA%3Finfer%3Dfalse`,
            function (result) {
                $("#cor-lab-results").show();
                $("#cor-lab-results").empty();

                var bindings = result["results"]["bindings"];
                for (var i in bindings) {
                    var label = bindings[i]["label"]["value"];
                    var uri = bindings[i]["uri"]["value"];

                    $("#cor-lab-results")
                        .append(`<div class="cor-lab-result" onclick="setCORUri('${uri}', '${label}')">
                                    ship: ${label}
                                </div>`);
                }
            }
    ...
    window.prev_query_time = query_time;
}

function setCORUri(uri, label) {
    $("#cor-lab-results").hide();
    $("#cor-uri-input").val(uri);
    $("#cor-lab-input").val(label);
}
